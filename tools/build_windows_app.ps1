...
$appDir = Join-Path $dist 'NaverMonitorApp'
$setupPayload = Join-Path $dist 'setup_payload'
$zipPath = Join-Path $setupPayload 'naver-monitor-app.zip'
$installerBat = Join-Path $setupPayload 'install_from_package.bat'
$setupExe = Join-Path $dist 'NaverMonitorSetup.exe'
$launcherSource = Join-Path $root 'tools\windows_launcher\NaverMonitorLauncher.cs'
$installerSource = Join-Path $root 'tools\windows_launcher\NaverMonitorInstaller.cs'
$launcherExe = Join-Path $appDir 'NaverMonitorLauncher.exe'
...
& $csc /nologo /target:winexe /out:$setupExe /r:System.Windows.Forms.dll /resource:$zipPath,naver-monitor-app.zip $installerSource
